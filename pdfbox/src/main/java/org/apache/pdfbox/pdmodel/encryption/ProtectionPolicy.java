...
public abstract class ProtectionPolicy {
    ...
    private static final String DEFAULT_ENCRYPTION_ALGORITHM = "RC4";

    private int                 encryptionKeyLength          = DEFAULT_KEY_LENGTH;

    /**
     * Encryption algorithm to use RC4 or AES, RC4 by default.
     */
    ...
    public void setEncryptionAlgorithm(final String encryptionAlgorithm) {
        if (!("RC4".equalsIgnoreCase(encryptionAlgorithm) || "AES".equalsIgnoreCase(encryptionAlgorithm))) {
            throw new IllegalArgumentException("Unsupported encryption algorithm '" + encryptionAlgorithm + "' value must be RC4 or AES!");
        }
    ...
    /**
     * set the length in (bits) of the secret key that will be used to encrypt document data. The default value is 40 bits, which provides a low security level but is compatible with old versions of
     * Acrobat Reader.
     * 
     * @param l
