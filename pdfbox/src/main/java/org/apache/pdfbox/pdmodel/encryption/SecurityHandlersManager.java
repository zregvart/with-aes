...
/**
 * This class manages security handlers for the application. It follows the singleton pattern. To be usable, security managers must be registered in it. Security managers are retrieved by the
...
public class SecurityHandlersManager {
    ...
    /**
     * hashtable used to index handlers regarding their name. Basically this will be used when opening an encrypted document to find the appropriate security handler to handle security features of the
     * document.
     */
    private Hashtable handlerNames         = null;

    /**
     * Hashtable used to index handlers regarding the class of protection policy they use. Basically this will be used when encrypting a document.
     */
    ...
    private SecurityHandlersManager() {
        ...
        try {
            this.registerHandler(StandardSecurityHandler.FILTER, StandardSecurityHandler.class, StandardProtectionPolicy.class);
            this.registerHandler(PublicKeySecurityHandler.FILTER, PublicKeySecurityHandler.class, PublicKeyProtectionPolicy.class);
    ...
    public SecurityHandler getSecurityHandler(final ProtectionPolicy policy) throws BadSecurityHandlerException {
        ...
        final Object found = handlerPolicyClasses.get(policy.getClass());
        if (found == null) {
        ...
        try {
            final Constructor c = handlerclass.getDeclaredConstructor(argsClasses);
            final SecurityHandler handler = (SecurityHandler) c.newInstance(args);
            return handler;
        } catch (final Exception e) {
            e.printStackTrace();
    ...
    public SecurityHandler getSecurityHandler(final String filterName) throws BadSecurityHandlerException {
        final Object found = handlerNames.get(filterName);
        if (found == null) {
     ...
     * 
     * If the security handler was already registered an exception is thrown. If another handler was previously registered for the same filter name or for the same policy name, an exception is thrown
     * 
     * @param filterName
     *            The name of the filter.
     * @param securityHandlerClass
     *            Security Handler class to register.
     * @param protectionPolicyClass
